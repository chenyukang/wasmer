...
impl Wasi {
    ...
    pub fn execute(
        ...
        let start = instance.exports.get_function(entry_func)?;
        let invoke_args = mapping_function_args(&start, &args)?;
        let result = start.call(&invoke_args);
        ...
        .with_context(|| format!("failed to run WASI `{}` function", entry_func))
    }
}
